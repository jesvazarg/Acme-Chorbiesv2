...
public interface ChirpRepository extends JpaRepository<Chirp, Integer> {
	...
	@Query("select c from Chirp c where c.sender.id=?1 and c.copy=false and c.recipients.size<2")
	Collection<Chirp> findChirpsSentByManagerId(int managerId);
	...
	@Query("select c from Chirp c where c.sender.id=?1 and c.copy=false and c.recipients.size>1")
	Collection<Chirp> findBroadcastsSentByManagerId(int managerId);
